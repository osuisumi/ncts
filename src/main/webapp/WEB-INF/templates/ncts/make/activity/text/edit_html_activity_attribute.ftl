<#macro editHtmlActivityAttributeFtl activity>
	<form id="saveActivityAttributeForm" action="${ctx }/make/activity/${(activity.id)!}" method="put">
		<ul class="g-addElement-lst g-addCourse-lst">
			<li class="m-addElement-item">
				<div class="ltxt">观看次数：</div>
				<div class="center">
					<div class="m-pbMod-ipt">
						<input type="text" name="activity.attributeMap[view_num].attrValue" placeholder="输入观看次数" class="u-pbIpt {digits:true, min:0}" value="${(activity.attributeMap.view_num.attrValue)!}">
					</div>
				</div>
			</li>
