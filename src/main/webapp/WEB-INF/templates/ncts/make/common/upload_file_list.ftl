<#macro uploadFileListFtl relationId relationType="" paramName="fileInfos" divId="fileDiv" btnTxt="上传文件" fileNumLimit=0 fileTypeLimit="" paramType='list'>
	<input type="hidden" class="paramType" value="${paramType }">
...
//        	});
	    ...
	    function initFileParam(divId) {
	    	...
	    	$list.find('.fileLi.success').each(function(i) {
	    		...
	    		var paramType = $('#'+divId+' .paramType').val();
	    		if(paramType == 'list'){
