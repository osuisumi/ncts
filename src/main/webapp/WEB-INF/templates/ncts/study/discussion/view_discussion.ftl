...
<@discussionPostsDirective discussionRelationId=discussionRelation.id page=pageBounds.page!1 limit=10 orders=(orders)!'CREATE_TIME.DESC'>
						...
						<div class="g-mn-mod">
							<div class="m-community-box">
								<div class="m-community-disc m-community-disc-dtl">
									<a href="${ctx}/${cid!}/study/course/discussion?limit=10&paramMap[relationId]=${cid!}&paramMap[relationType]=courseStudy" class="m-back-comList">返回列表&gt;</a>
								</div>
								<div class="u-con-ul m-course-detail-cont ">
									...
									<div class="u-btn-info">
										<p class="u-lt">
											...
											发表于${TimeUtils.formatDate(discussion.createTime,'yyyy-MM-dd HH:mm')}&nbsp;&nbsp;&nbsp;&nbsp;
											<#--浏览次数：<span>${(discussion.discussionRelations[0].browseNum)!}</span>-->
										</p>
									...
									<p class="u-cou-det-res">
										...
										<span><i id="totalPostNum">${(paginator.totalCount)!0 }</i>条回复</span>
									</p>
								</div>
													...
													<div class="c-info">
														...
														<div class="ag-opa">
															...
															<div class="reslut">
																...
																<#if ((post.creator.id)!'' == (Session.loginer.id)! || hasTeachRole)>
																	<i class="line">|</i>
								...
								<form id="viewDiscussionForm" action="${ctx}/${cid!}/${role }/course/discussion/${discussionId}">
...
<form id="saveDiscussionPostForm" action="${ctx }/${cid!}/${role }/course/discussion/post/save">
...
<script>
	...
	function saveDiscussionPost(){
		...
		$.post("${ctx}/${cid!}/${role }/course/discussion/post/save",{
				...
				success: function(data){
					if(data.responseCode == '00'){
						alert('发表成功', function(){
							...
							$(btn).closest('.childPostDiv').find('.childPostTextarea').eq(0).val('');
	...
	function loadChildPost(postId){
		...
		$('#listChildPostDiv_'+postId).load('${ctx}/${cid!}/${role }/course/discussion/post/child?paramMap[mainPostId]='+postId);
	}
	
	function rankPost(param){
		if(param =='createTime'){
			window.location.href = "${ctx}/${cid!}/${role }/course/discussion/${discussionId}?orders=CREATE_TIME.DESC";
		}else{
			window.location.href = "${ctx}/${cid!}/${role }/course/discussion/${discussionId}?orders=SUPPORT_NUM.DESC";
		}
	...
	*/
	...
	function editDiscussionPost(postId, mainPostId){
		mylayerFn.open({
	        ...
	        fix: true,
	        area: [600, 300],
	        content: '${ctx}/${cid!}/${role }/course/discussion/post/'+postId+'/edit?mainPostId='+mainPostId,
	    });
	}
	...
	function deletePost(id, mainPostId, obj){
		confirm('是否删除此回复?',function(){
			...
			$('#deletePostForm').attr('action','${ctx}/${cid!}/${role }/course/discussion/post/'+id);
