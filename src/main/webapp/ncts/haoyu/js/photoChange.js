...
studySelectAct = {
    ...
    star : function(indexs){
        ...
        var $actList = $optBox.find('.m-studyAct-lst'),
            ...
            $listBox = $optBox.find(".list-box"),
            $listBox_w = $listBox.width(),
            $actItem_w = $listBox_w / 3;
        ...
        if(lengths == 1){
            $actItem.width($listBox_w);
            $actItem_w = $listBox_w;
        }
        else if(lengths == 2){
            $actItem.width($listBox_w / 2);
            $actItem_w = $listBox_w / 2;
        }
        else{
            $actItem.width($actItem_w);
            $actItem_w = $listBox_w / 3;
        }

        $actList.width($actItem_w * lengths);

        //获取单个宽度
        var itemWidth = $actItem_w;
